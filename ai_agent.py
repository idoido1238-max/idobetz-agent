# -*- coding: utf-8 -*-
# idobetz — שרת מלא: סוכן AI + אוטומציית הזמנות + מדריכים + מבצע
import os
import json
import time
...
# ============================================
# הגדרות — environment variables ב-Railway
# ============================================
...
# בסיס הקישורים למדריכים ב-GitHub
MANUALS_BASE = "https://raw.githubusercontent.com/idoido1238-max/idobetz-agent/master/"

ISTORES_HEADERS = {
    "x-token": ISTORES_API_KEY,
    "Company-Id": ISTORES_API_KEY,
    "Content-Type": "application/json",
    "User-Agent": "idobetz-automation/1.0",
    "Accept": "application/json"
}

# ============================================
# מיפוי מק"ט → מדריך הפעלה
# ============================================
PRODUCT_MANUALS = {
    "akamado21": "kamado21_manual.pdf",
    "akamado16": "kamado16_manual.pdf",
    "akamado13": "kamado13_manual.pdf",
    "adivmk6618": "icecream_mk6618_manual.pdf",
    "adivsm212": "slush_sm212_manual.pdf",
    "adivpf1r": "thai_icecream_pf1r_manual.pdf"
}

# מק"טים שמזכים במבצע (מעשנות קמאדו)
KAMADO_SKUS = ["akamado21", "akamado16", "akamado13"]

# מצב הסוכן AI — כבוי כברירת מחדל
agent_active = False
...
def send_file(phone, file_url, filename, caption=""):
    ...
    data = json.dumps({
        "chatId": chat_id, "urlFile": file_url,
...
def handle_new_order(order):
    ...
    # 3. מדריכי הפעלה — לפי מק"ט
    has_kamado = False
    for p in products:
        sku = get_sku(p)
        for known_sku, manual in PRODUCT_MANUALS.items():
            if known_sku.lower() in sku:
                manual_url = MANUALS_BASE + manual
                send_file(phone, manual_url, manual,
                          "📖 מדריך הפעלה ותחזוקה למוצר שלך")
                time.sleep(2)
                if known_sku in KAMADO_SKUS:
                    has_kamado = True
                break

    # 4. מבצע — רק לרוכשי קמאדו
    if has_kamado:
...
# סוכן AI
# ============================================
SYSTEM_PROMPT = """אתה סוכן AI עבור Idobetz - חנות ציוד מטבח מקצועי (סכיני שף, מעשנות קמאדו, מכונות ברד וגלידה, ועוד).
...
בכל התעניינות במוצר הוסף "תתחדשו!" בסוף.
אל תציין שאתה בוט אלא אם שואלים."""

def ask_claude(phone, msg):
    if not CLAUDE_API_KEY:
        return "מצטערים, השירות אינו זמין כרגע."
    ...
    data = json.dumps({
        ...
        "system": SYSTEM_PROMPT, "messages": conversation_history[phone]
    }, ensure_ascii=False).encode("utf-8")
    ...
    def do_POST(self):
        ...
        if event == "new_order" or "new_order" in str(event):
            ...
            try:
                ...
                print("Order handling error: " + str(e))
            self.send_response(200); self.end_headers()
            self.wfile.write(b'{"success":true}'); return

        # === Webhook מ-GREEN-API: הודעה נכנסת ===
        if data.get("typeWebhook") == "incomingMessageReceived":
            ...
            if msg and phone:
                ...
                if is_owner:
                    if "סוכן ON" in msg or "סוכן on" in msg.lower():
                        agent_active = True
                        send_whatsapp(phone, "✅ הסוכן הופעל!")
                    elif "סוכן OFF" in msg or "סוכן off" in msg.lower():
                        agent_active = False
...
if __name__ == "__main__":
    ...
    print("Manuals: " + str(len(PRODUCT_MANUALS)) + " products mapped")
    print("=" * 50)
